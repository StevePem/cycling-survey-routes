...
for (i in 1:nrow(routes_sf)) {
  ...
  print(paste(Sys.time(), "| Finding network route for trip", i, "of", nrow(routes_sf)))
  ...
  if (nrow(vertices) > 1) {
    ...
    # write to output datframe
    routes_networked$network_nodes[i] <- toString(node_list)
    routes_networked$network_edges[i] <- toString(edge_list)
